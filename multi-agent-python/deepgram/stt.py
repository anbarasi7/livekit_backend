...
class SpeechStream(stt.SpeechStream):
    ...
    def _process_stream_event(self, data: dict) -> None:
        ...
        # for more information about the different types of events
            ...
            # Check if any word in any alternative has low confidence
            # alt = data.get("channel", {}).get("alternatives", [])
            for alt in data.get("channel", {}).get("alternatives", []):
                for word in alt.get("words", []):
                    print(word)
                    print(word.get("confidence", 1.0))
                    if word.get("confidence", 1.0) < 0.7:
                        print(f"[Deepgram Low Confidence] Word: '{word.get('word')}' Confidence: {word.get('confidence')}")
            ...
            if should_fallback:
                alts = [
                    stt.SpeechData(
                        ...
                        end_time=0,
                        confidence=0.0,
                        text="reframe this sentence: I couldn't hear, could pls repeat or come closer. Don't say anything else just only the reframed sentence",
                    )
                ]
